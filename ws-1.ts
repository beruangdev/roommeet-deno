  ...
  socket.onopen = () => {
    if (!id && !room) {
      ...
      for (const _id in peers[room]) {
        console.log("peers[room][_id]", typeof peers[room][_id], peers[room][_id])
        const peerObject = peers[room][_id];
        if (_id !== id && peerObject && typeof peerObject?.readyState === "number") {
          wsSend(peerObject, {
            type: "initReceive",
            data: { id },
          });
