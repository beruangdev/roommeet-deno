...
const handler: Handler = ({ request, user }) => {
  ...
  socket.onopen = () => {
    ...
    peers[room]["password"][user_id] = socket;
...
export const enterRoom: Handler = ({ body }) => {
  try {
    if (!body) {
    ...
    if (!room || !user_id) {
      throw new HttpError(400, "Room name or user identifier is missing from request body");
    }
